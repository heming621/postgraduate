...
*/
object eclat{
	val minSup = (0.02*990002).toInt//(0.05*8124).toInt//(0.3*100000).toInt//(0.25*49046).toInt //0.5*6
        val LDATA_PATH = "/home/mountDir/downloads/data/kosarak.dat"//"/home/zhm/sparkEclatV1/data/mushroom.dat"//"/home/mountDir/downloads/data/pumsb_star.dat
	def eclat(_prefix:Set[Int], _x:Map[Int,Set[Int]], _xx:mutable.Map[Int,Set[Int]], result:mutable.Map[Set[Int], Int]):mutable.Map[Set[Int],Int] = {//:Unit = {
		...
		while(!xx.isEmpty){
		    ...
		    xx = xx.dropRight(1)            
                    //println(s"xx.size:${xx.size}")
	            if(isup >= minSup && (_x.keys.toSet.contains(itemtid._1) | !prefix.isEmpty)){ //zi只对出现在_x里面的item生成频繁集//zi取最后一个item_tid，如果满足最小支持度，并与剩余的(itemSet,TIDs)做交集。
			...
			if(!(_x.keys.toSet & prefixRcs).isEmpty && prefixRcs.size < 7)     //zi 前缀prefix在小集合_x里，出现过的才继续递归，继续生成维度更高的项集。
	...
	def main(args:Array[String]){
	        ...
	        //var x:mutable.Map[Int,Set[Int]] = mutable.Map(66->Set(1,2,4,6), 11->Set(3,4,5,6), 55->Set(1,2,4,5,6))//mutable.Map(11->Set(3,4,5,6), 55->Set(1,2,4,5,6))//var x = xx
                ...
                println("Start FIM.")
        	eclat(Set(), x.toList.toMap, xx, results)             //zi x.toList.toMap, for Spark
        	//eclat(Set(), x.toList.toMap.filter(x=>x._2.size>=minSup), xx, results)
                println("\n")
        	println(results)
