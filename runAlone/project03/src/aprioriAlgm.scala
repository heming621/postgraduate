...
object aprioriAlgm{
    ...
    def main(args: Array[String]){
        ...
        val DATA_PATH = "/home/mountDir/downloads/data/mushroom.dat"
        val minSup = 0.15
        ...
        while(freqItems.size > 0){ // && k <= 4){ //??freqItems.length > 0?? freqItems.length != null ??
                ...
                println(s"This iteration cost ${(tmpE2-tmpS2)/1000 + (tmpE1-tmpS1)/1000}s\n")
                k += 1
        }
        var timee = System.currentTimeMillis()
        println(s"\nTimecost:${(timee-times)/1000}s")
        //FIs: ArrayBuffer[Array[(Set[Int], Int)]]
        val file = new File("/home/zhm/project03/FIs.txt")
