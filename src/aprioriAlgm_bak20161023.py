...
def runApriori(data_iter, minSupport, minConfidence):
    ...
    while(currentLSet != set([])):
        ...
        print "%s-FIs finished! And the %s-Freq is %s. - %s " % (k, k, len(currentCSet), round(time.clock(),2)), "\n"
        k = k + 1
...
def printResults(items, rules):
    ...
    for item, support, count in sorted(items, key=lambda (item, support, count): support):
        print "item: %s , %.3f" % (str(item), support)
    print "\n------------------------ RULES:"
    for rule, preC, setC, lenOfT, confidence in sorted(rules, key=lambda (rule, preC, setC, lenOfT, confidence): confidence):
        pre, post = rule
